...
def shorten_link(token, url):
    bitly_response = requests.post('https://api-ssl.bitly.com/v4/bitlinks',
                                   json={"long_url": url},
...
def count_clicks(token, bitlink):
    bitly_response = requests.get(f'https://api-ssl.bitly.com/v4/bitlinks/{bitlink}/clicks/summary', 
                                  headers={"Authorization": f"Bearer {token}"})
...
def is_bitlink(token, url):
    bitly_response = requests.get(f'https://api-ssl.bitly.com/v4/bitlinks/{url}', 
                                  headers={"Authorization": f"Bearer {token}"})
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Программа делает Bitly ссылки и показывает количество их использования')
